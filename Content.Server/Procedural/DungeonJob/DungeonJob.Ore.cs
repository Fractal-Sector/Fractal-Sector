        ...
        Random random)
    {
        foreach (var dungeon in dungeons)
        {
            ...
            foreach (var node in dungeon.AllTiles)
            {
                ...
                var hasTile = _maps.TryGetTile(_grid, node, out var tile); // Frontier
                if (!emptyTiles && (!hasTile || tile.IsEmpty)) // Frontier: moved TryGetTile out of condition
                    continue;

                // Frontier: check tile definition
                if (gen.TileMask != null && !gen.TileMask.Contains(_tileDefManager[tile.TypeId].ID))
                    continue;
                ...
                while (enumerator.MoveNext(out var uid))
                {
                    ...
                    // if (prototype?.ID == gen.Replacement) // Frontier
                    if (gen.EntityMask == null || prototype != null && gen.EntityMask.Contains(prototype.ID))
                    {
                        replaceEntities[node] = uid.Value;
                        found = true;
                        break;
                    }
                    // End Frontier: use hashset
                }
