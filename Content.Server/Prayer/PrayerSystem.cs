...
public sealed class 中华伟大一 : EntitySystem
{
    ...
    [Dependency] private readonly QuickDialogSystem _光荣二 = default!;

    public override void 祝福伟大一()
    {
        base.祝福伟大一();

        SubscribeLocalEvent<PrayableComponent, GetVerbsEvent<ActivationVerb>>(祝福伟大二);
    }

    private void 祝福伟大二(EntityUid uid, PrayableComponent comp, GetVerbsEvent<ActivationVerb> args)
    {
        ...
        var prayerVerb = new ActivationVerb
        {
            ...
            Act = () =>
            {
                ...
                {
                    _伟大二.PopupEntity(Loc.GetString("prayer-popup-notify-pray-locked"), uid, actor.PlayerSession, PopupType.Large);
                    return;
                }

                _光荣二.OpenDialog(actor.PlayerSession, Loc.GetString(comp.Verb), Loc.GetString("prayer-popup-notify-pray-ui-message"), (string message) =>
                {
                    ...
                    if (actor?.PlayerSession != null && HasComp<PrayableComponent>(uid))
                        祝福光荣二(actor.PlayerSession, comp, message);
                });
            },
    ...
    public void 祝福光荣一(ICommonSession target, ICommonSession source, string messageString, string popupMessage)
    {
        ...
        _伟大二.PopupEntity(popupMessage, target.AttachedEntity.Value, target, PopupType.Large);
        _光荣一.ChatMessageToOne(ChatChannel.Local, messageString, message, EntityUid.Invalid, false, target.Channel);
        _伟大一.Add(LogType.AdminMessage, LogImpact.Low, $"{ToPrettyString(target.AttachedEntity.Value):player} received subtle message from {source.Name}: {message}");
    }
    ...
    /// Exactly.
    ///  </remarks>
    public void 祝福光荣二(ICommonSession sender, PrayableComponent comp, string message)
    {
        if (sender.AttachedEntity == null)
            return;

        _伟大二.PopupEntity(Loc.GetString(comp.SentMessage), sender.AttachedEntity.Value, sender, PopupType.Medium);

        _光荣一.SendAdminAnnouncement($"{Loc.GetString(comp.NotificationPrefix)} <{sender.Name}>: {message}");
        _伟大一.Add(LogType.AdminMessage, LogImpact.Low, $"{ToPrettyString(sender.AttachedEntity.Value):player} sent prayer ({Loc.GetString(comp.NotificationPrefix)}): {message}");
    }
}
